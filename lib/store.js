...
export async function upsertAccount(account) {
  ...
  await pool.query(
    ...
    [
      ...
      // BIGINT column: must be a plain number, not a Date (pg would serialize
      // a Date to an ISO string and Postgres would reject it for bigint).
      Math.floor(Number(account.expiresAt) || 0),
